...
Config = {
    ...
    "OPTIMIZATION_CRITERIA": {
        "min_z": {
            "enabled": True,
            "range": np.arange(0.0, 3.05, 0.1),   # 31 values: z-score threshold
        },
        "close_mean": {
            "enabled": True,
            "range": np.arange(0.0, 0.35, 0.05),   # 7 values: fee floor
        },
        "upside_probability": {
            "enabled": False,
...
def compute_trades(df, horizon, min_change_pct=0.0, min_z=None, min_upside_prob=None):
    ...
    for _, row in df.iterrows():
        ...
        # z-score filter: z = |predicted move| / uncertainty
        if min_z is not None and min_z > 0:
            z = abs(pred_change_pct) / std_pct if std_pct > 0 else float("inf")
            if z < min_z:
                signals.append(None)
                continue
    ...
    for h in horizons:
        ...
        for combo in itertools.product(*axis_values):
            ...
            kwargs = {
                ...
                "min_z": params.get("min_z", None) if params.get("min_z", 0) > 0 else None,
...
def plot_equity_charts(df, optimized_df, run_dir: Path):
    ...
    for _, row in optimized_df.iterrows():
        ...
        kwargs = {}
        if "best_min_z" in row.index:
            mz = row["best_min_z"]
            kwargs["min_z"] = mz if mz > 0 else None
...
if __name__ == "__main__":
    ...
    shutil.copy(__file__, run_dir / "model_performance_zscore.py")
